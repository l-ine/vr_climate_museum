...
namespace DialogueEditor
{
    public class ConversationManager : MonoBehaviour
    {
        ...
        public void Awake()
        {
            //Debug.Log("ConversationManager Awake called");
            // Destroy myself if I am not the singleton
            if (Instance != null && Instance != this)
            {
                GameObject.Destroy(this.gameObject);
                //Debug.Log("ConversationManager Awake: destroyed duplicate instance.");
            }
            Instance = this;
        ...
        private void Update()
        {
            //Debug.Log("ConversationManager Update called, mState: " + m_state);
            switch (m_state)
            {
        ...
        {
            //Debug.Log("ConversationManager SetState | mState: " + m_state + " | newState: " + newState);
            // Exit
            switch (m_state)
            {
                case eState.TransitioningOptionsOff:
        ...
        private void TransitionOptionsOff_Update()
        {
            m_stateTime += Time.deltaTime;
            float t = m_stateTime / TRANSITION_TIME;
            //Debug.Log("ConversationManager, transition options off, t: " + t);
            ...
            if (t > 1)
            {
                ...
                //Debug.Log("ConversationManager, transition options off, m_selectedOption: " + m_selectedOption);
                if (m_selectedOption == null)
                {
                    //Debug.Log("ConversationManager, transition options off, end conversation case.");
                    EndConversation();
                    return;
                }

                SpeechNode nextSpeech = GetValidSpeechOfNode(m_selectedOption);
                //Debug.Log("ConversationManager, TransitionOptionsOff_Update | next speech: " + nextSpeech + " | m_selectedOption: " + m_selectedOption);
        ...
        {
            //Debug.Log("Manager.OptionSelected(), Option selected: " + option.Text);
            //Debug.Log("Manager.OptionSelected(), Option event: " + option.Event);
            m_selectedOption = option;
            DoParamAction(option);
            if (option.Event != null)
            {
                //Debug.Log("option.Event is not null, invoking it.");
                option.Event.Invoke();
            }
            SetState(eState.TransitioningOptionsOff); // do not show the answer options anymore
        }
        ...
        {
            //Debug.Log("Manager.DoParamAction(), node has " + node.ParamActions.Count + " actions.");
