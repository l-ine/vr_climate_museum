...
public class UpdateEnvironment : MonoBehaviour
{
    ...
    //public LogSliderChanges logSliderChanges;
    //public CMDinterface cmdInterface;
    public LoadSimulator simulationRenderer;
    ...
    //public GameObject GlobalTempIndicator; // The GameObject containing the TextMeshPro object
    ...
    {
        //set2022();
        UnityEngine.Debug.Log("UpdateEnvironment: Start()");
    ...
    public void Update()
    {
        ...
        if (!this.busy && checkForChange(this.lastURL, this.currURL)) // prevents executions whilst previous adaptions are still made
        {
            ...
            this.busy = true; // prevents parallel runs of the routine
                              // save all slider changes the user has made in a csv
                              //logSliderChanges.SaveSliderValues(currURL);
            UnityEngine.Debug.Log("UpdateEnvironment: update(): start coroutine readoutandapply");
    ...
    public bool checkForChange(string lastURL, string currURL)
    {
        ...
        // has 3d slider switched?
        if (yearSelector != prevYearSelector)
        {
            UnityEngine.Debug.Log("UpdateEnvironment: checkforchange(): Year selector changed from " + prevYearSelector + " to " + yearSelector);
            return true;
        }
    ...
    private IEnumerator readOutAndApplyValues()
    {
        ...
        //{
            UnityEngine.Debug.Log("UpdateEnvironment: readOutAndApplyValues(): env update API call starts");
    ...
    {
        //UnityEngine.Debug.Log("chooseYear() called with yearPredictions: " + string.Join(", ", yearPredictions));
        //TextMeshProUGUI textMeshPro = GlobalYearIndicator.GetComponent<TextMeshProUGUI>();
        //TextMeshProUGUI textMeshProTemp = GlobalTempIndicator.GetComponent<TextMeshProUGUI>();
        ...
        UnityEngine.Debug.Log("UpdateEnvironment: apply() called with tempProg: " + tempProg);
        this.active2100 = true;
        apply(tempProg);
        // display the temp increase
        //textMeshProTemp.text = "+" + tempProg.ToString("0.0") + "°C";
    }

    public void apply(float temp2100)
    {
        UnityEngine.Debug.Log("UpdateEnvironment: apply() called with temp2100: " + temp2100 + "active2100 = " + this.active2100);
        ...
        if (this.active2100)
        {
            UnityEngine.Debug.Log("UpdateEnvironment: active2100 true; prognosis is applied to the environment");
        ...
        this.busy = false;
        UnityEngine.Debug.Log("UpdateEnvironment: apply(): environment updated, busy set to false");
    }
